...
import Store from "./contracts/Store copy 4";
...
class App extends Component {
   ...
   buyCoke=()=>{
   console.log("##성공")
   this.state.storeInstance.BuyCoke(this.state.num,{
   from:this.state.myAccount,
   value:this.state.web3.toWei(0.01*this.state.num,"ether")
      })
    }

    sellCoke=()=>{
       this.state.storeInstance.sellCoke(this.state.web3.towei(0.001, "ether"),{
          from : this.state.myAccount
       })
    }
   ...
   render() {
      return (
         ...
         <div>우진이의 사이다 개수 : {this.state.myCokeNum} <button onClick={this.getMyCoke}>갱신하기</button> </div>
